...
%%% Richard She
%%% December 07 2016
function [] = fwselection(masterPath,savePath,pcutoff,p)
...
%%% Load in depedencies.
cd(masterPath);
load([masterPath,'filename.mat']);
load([masterPath,'trait.mat']);
...
prefix = [savePath,filename{p}(1:length(filename{p})-4)];
save([prefix,'_cutoff_',num2str(pcutoff),'_b.mat'],'b','-v7.3'); 
...
nVariants = sum(stats.PVAL>5.2);
bPos_filtered = find(-log10(stats.PVAL) > 5.2);
