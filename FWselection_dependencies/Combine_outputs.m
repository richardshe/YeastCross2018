...
pathname = '/Users/richardshe/Dropbox/Jarosz Lab Project/Presentations/YeastCross/FWselection_dependencies';
if pathname(length(pathname)) ~= '/'
    pathname = [pathname,'/'];
end
load([pathname,'trait.mat']) %%%% Your Raw 1x1152 trait values
...
pathname = '/Users/richardshe/Documents/F6cross_fwselection/';
cd(pathname);
files = dir('*0.01_b_trimmedGenotype120716*.mat');
filename = cell(length(files),1);
for i = 1:length(files)
    filename{i} = files(i).name;
    temp = strsplit(filename{i},'-');
...
for fileIndex = 1:length(files)
    ...
    history = b.history;
    load([pathname,files2(fileIndex).name]);
    b.history = history;
    ...
    if ~strcmp(time{fileIndex},'0h')
        ...
        bPos_filtered = find(-log10(b.stats.PVAL) > cutoff2(fileIndex));
        ...
        devLenient = sum(varianceExplained(-log10(b.stats.PVAL(b.bPos)) > cutoff2(fileIndex)));
